...
## Approach
All course material, both for lectures and practical sessions, is provided as Pluto notebooks. A self-contained project environment is provided, so in principle, you'll need to do the configuration only once, and then you can always work in the same way without any issues or missing dependencies. After downloading the repository, open the one-page guide [Julia environments: one course, one reproducible toolbox](setup/julia-environments.html) in your browser. It explains what an environment is, how this course uses it, and how it differs from the `.julia` folder. The [Julia manual](https://docs.julialang.org/en/v1/manual/code-loading/#Environments-1) provides more technical detail.

The course setup requires Julia v1.10.x. The environment is resolved for Julia 1.10.0, the release on the CDN machines, and runs unchanged on every later 1.10.x. On personal Windows, MacOS and Linux computers, install [Juliaup](https://github.com/JuliaLang/juliaup) first and then install the 1.10 channel with `juliaup add 1.10`, which gives you the newest 1.10.x. On the CDN Windows 11 Enterprise machine, use the installed Julia 1.10 executable at `C:\Program Files\Julia-1.10\bin\julia.exe`. The setup scripts will stop with an error when run with another Julia version.
